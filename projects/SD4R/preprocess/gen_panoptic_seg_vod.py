...
if __name__ == '__main__':
    ...
    datasets = 'VoD' # NOTE: cyclist is bicycle with person, thus here is difference
    if datasets =='VoD': require_cat = [0, 1, 2] # person bicycle car
    if datasets =='TJ4D': require_cat = [0, 1, 2, 8] # person bicycle car truck
    save_panoptic_masks_dir = 'data/VoD/segmentation'
    root_path='data/VoD/radar_5frames/testing/image_2'
    ImageSets = 'data/VoD/radar_5frames/ImageSets/full.txt'
    ...
    with torch.no_grad():
        for image_idx in range(len(ImageSets)):
            ...
            image_path = os.path.join(root_path, filename + '.jpg')
